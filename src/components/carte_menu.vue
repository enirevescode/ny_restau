<template>
 
    <Card_menu_row/>
 
</template>
<script>
//IMPORT SRCES
import menu from '@/menu.js';
import { onMounted } from 'vue';

//COMPONENTS
  import Card_menu_row from '@/components/Card_menu_row.vue'

  export default {

   name: "carte_menu",
   components: { Card_menu_row, menu },

   setup() {

    class CarteAppetizer {
      constructor (id, category, plat, prix, ingredient) {
        this.id = id
        this.category = category
        this.plat = plat
        this.prix = prix
        this.ingredient = ingredient
      }
    }

    let dataAppetizer = []

    const filtreAppetizer = () => {
      for ( const CarteAppetizer of menu) {

      }
    }


    
   }
   
   
  //  data: () => ({

  //  menus : [
  //         {
  //         id: 1,
  //         category: "appetizer",
  //         plat: "LEMON AND GARLIC GREEN BEANS",
  //         prix: "15.00",
  //         ingredient: "Lemon / Garlic / Beans",
  //           },
  //           {
  //         id: 2,
  //         category: "appetizer",
  //         plat: "BACON-WRAPPED SHRIMP WITH GARLIC",
  //         prix: "21.50",
  //         ingredient: "Bacon / Shrimp / Garlic",
  //           },
  //           {
  //         id: 3,
  //         category: "appetizer",
  //         plat: "LAMB-BEEF KOFKA SKEWERS WITH TZATZIKI",
  //         prix: "18.50",
  //         ingredient: "Lamb / Wine / Butter",
  //           },
  //           {
  //         id: 4,
  //         category: "appetizer",
  //         plat: "IMPORTED OYSTERS GRILL (5 PIECES)",
  //         prix: "20.00",
  //         ingredient: "Oysters / Veggie / Ginger",
  //           },
  //           {
  //           id: 5,
  //           category: "appetizer",
  //         plat: "LEMON AND GARLIC GREEN BEANS",
  //         prix: "15.00",
  //         ingredient: "Lemon / Garlic / Beans",
  //           },
  //           {
  //         id: 6,
  //         category: "appetizer",
  //         plat: "BACON-WRAPPED SHRIMP WITH GARLIC",
  //         prix: "21.50",
  //         ingredient: "Bacon / Shrimp / Garlic",
  //           },
  //           {
  //         id: 7,
  //         category: "appetizer",
  //         plat: "LAMB-BEEF KOFKA SKEWERS WITH TZATZIKI",
  //         prix: "18.50",
  //         ingredient: "Lamb / Wine / Butter",
  //           },
  //           {
  //         id: 8,
  //         category: "appetizer",
  //         plat: "IMPORTED OYSTERS GRILL (5 PIECES)",
  //         prix: "20.00",
  //         ingredient: "Oysters / Veggie / Ginger",
  //           },
  //           {
  //         id: 9,
  //         category: "main",
  //         plat: "Entrecôte - French Fries",
  //         prix: "20.00",
  //         ingredient: "Beef / Veggie / Potatoes",
  //           },
  //           {
  //         id: 10,
  //         category: "main",
  //         plat: "Cassoulet - Saucisses",
  //         prix: "20.00",
  //         ingredient: "Beans / Saucisses",
  //           },
  //           {
  //           id: 11,
  //         category: "main",
  //         plat: "Entrecôte - French Fries",
  //         prix: "20.00",
  //         ingredient: "Beef / Veggie / Potatoes",
  //           },
  //           {
  //         id: 12,
  //         category: "main",
  //         plat: "Cassoulet - Saucisses",
  //         prix: "20.00",
  //         ingredient: "Beans / Saucisses",
  //           },
  //           {
  //           id: 13,
  //         category: "main",
  //         plat: "Moules - French Fries",
  //         prix: "20.00",
  //         ingredient: "Moules / Veggie / Potatoes",
  //           },
  //           {
  //         id: 14,
  //         category: "main",
  //         plat: "Saucisses - Lentilles",
  //         prix: "20.00",
  //         ingredient: "Lentilles Vertes / Saucisses",
  //           },
  //         ]
  //     }),
   }
</script>
<style scoped>

</style>menu.js